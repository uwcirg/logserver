...
Directory layout:
tests/
├── data/                  ← JSON fixtures for each schema
│   ├── legacy_sample.json
│   ├── ltt_sample.json
│   ├── isacc_sample.json
...
│   └── ...
└── test_views.py          ← pytest suite and helper functions
tests/test_views.py exposes:
insert_mock_data(cur, data_list): truncates api.events, inserts rows.
query_events_view(cur): runs SELECT schema_version, occurred_at, user_id, event_type FROM api.events_view and returns raw results.
normalize_results(rows): formats Python datetime into YYYY-MM-DD HH:MM:SS[+00], appends +00 for non-legacy schemas.
run_test(data_list, expected_rows): helper that executes the above, asserts equality.
Fixtures in test_views.py load JSON files from tests/data and define expected tuples:
Key functions in test_views.py:
insert_mock_data(cur, data_list): truncates api.events, inserts JSON records.
query_events_view(cur): executes:
	SELECT schema_version, occurred_at, user_id, event_type
  			FROM api.events_view
ORDER BY occurred_at;
and returns the raw rows.
normalize_results(rows): formats (schema, datetime, user_id, event_type) tuples into strings YYYY-MM-DD HH:MM:SS, appending +00 for non-legacy schemas.
Fixtures:
Load JSON fixtures from tests/data/ via json.load(open(...)).
CSV-based expected results (legacy_expected.csv, ltt_expected.csv) read with a load_expected() helper.
Inline expected values for ISACC, dhair2, and COSRI tests.
Adding New Test Cases
Add a new JSON fixture under tests/data/, containing only fields your view uses: version, timestamps (event_time, occurred, or asctime), subject or array, action/tags/message, and source as needed.
Write a pytest function in test_views.py:
...
Commit & push your fixture and test; CI will run automatically.

Filtering Data via api.events_view
In psql:
-- Legacy only
SELECT *
  FROM api.events_view
...
Filtering Raw JSON Data (Optional)
-- Find all events by nested source.type
SELECT event->'source'->>'type' AS src, COUNT(*)
  FROM api.events
 GROUP BY src;
